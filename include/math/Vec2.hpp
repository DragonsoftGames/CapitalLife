...
public:
	...
	explicit Vec2(float p_x, float p_y) : x(p_x),
										  y(p_y)
	{
	}

	template <class X, class Y>
	explicit Vec2(X p_x, Y p_y) : x(static_cast<float>(p_x)),
								  y(static_cast<float>(p_y))
	{
	}

	// Copy constructor
	Vec2(const Vec2 &other) : x(other.x),
							  y(other.y)
	{
	}
	...
	void fixLerp(Vec2 target, float alpha, float distance)
	{
		const float invAlpha = 1.0f - alpha;
		if (std::abs(x - target.x) > distance)
			x = (x * invAlpha) + (target.x * alpha);
		if (std::abs(y - target.y) > distance)
			y = (y * invAlpha) + (target.y * alpha);
	}
